...
class Administracion extends MX_Controller {
	...
	public function __construct(){
		...
		//Idioma
		$this->lang->load('administracion', $this->_session->data->idioma);
	}
	...
	public function index()
	{
		$data = [
			...
			"conf"   => $this->_conf,
			"total"  => $this->madministracion->_getTotales(),
			"pagos"  => $this->madministracion->_getPayments(),
			"recauda"  => $this->madministracion->_getRecaud(),
		];

		$this->load->view('layouts/admin/Header',$data);
		$this->load->view('layouts/admin/Navbar',$data);
		$this->load->view('layouts/admin/Sidebar',$data);
		$this->load->view('Vadministracion',$data);
