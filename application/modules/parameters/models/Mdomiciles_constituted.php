...
class Mdomiciles_constituted extends CI_model
{
    ...
    public function _getDomicilesCons(){
        $cod =$this->_session->data->codcliente;
        $q = $this->db->query("SELECT * FROM domicilio_constituido WHERE codcliente = '$cod' ORDER BY
            fechareg DESC");
    ...
    public function _newDomicileCons($x){
        $data = array(
        ...
        $this->db->insert('domicilio_constituido',$data);
    ...
    public function _delDomicileCons($x){
        ...
        if(password_verify($x[1], $a->row()->pass)){
            ...
            $this->db->trans_begin();
            $this->db->where('idDomicilioCons = ',$x[0]);
            $this->db->delete('domicilio_constituido');
    ...
    public function _getFullData($x){
        $q = $this->db->query("SELECT * FROM domicilio_constituido WHERE idDomicilioCons = '$x'");
